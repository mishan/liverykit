...
const AERO = `- A stripe over the top of the car is barely seen from trackside, which looks at its side, so a style's colors must also be where a side view sees them. For a Gulf livery that is an orange ground-effect kit: the front splitter, the side skirts and the rear diffuser. Call find_space with { panel: <any panel of the bodywork>, aero: { heightMm: 300 } } alongside the rest. It returns the car's lowest panels all round as regions ready to use: add a color. It leaves the doors out, so the lettering on them keeps its background.
`;
const WHEELS = `- Paint the wheels, in the first draft: a fill on surfaces.rims in the style's accent color, orange for Gulf. surfaces.rims is the wheels, whatever texture describe_car shows it bound to, and a note calling that texture a placeholder, a decal or small does not change that. The stock dark rims disappear against the tyres. Leave it out only when describe_car says the car has no rims surface.
`;
...
- Panels are unwrapped every which way, and find_panels says how each one runs on the car in "axes": { x, y }, where x and y are the two directions of "at". A stripe along the car spans the axis that runs along it: at [0, 0.4, 1, 0.2] when x runs along the car, [0.4, 0, 0.2, 1] when y does. Getting it backwards paints a band across the car instead of a stripe down it. Check every panel a stripe crosses; neighboring panels are often turned differently. An axis find_panels could not measure clearly is null, with the reason in "unclear": a panel laid diagonally, or one whose two axes came out the same. Do not guess one; look at that panel with render_car before running a stripe on it, or use a panel whose axes are named. A stripe along the car runs its whole length, nose to tail, and over the top of the rear wing when the car has one, at the same place and width all the way, and over every panel its band crosses, a roof hatch or any small panel set into the bodywork included. Leave its width to find_space unless the brief sets one: without widthMm it sizes the stripe to the car's bodywork, a third of its width and 450 mm at most, which is bold on a GT car and narrower on a small car or a formula car's nose, and says what it chose. Do not work the pieces out yourself: call find_space with { panel: <any panel of the bodywork>, stripe: { offsetMm } }, where offsetMm is the band's centre from the car's centerline, left positive, and 0 for a centre stripe. It returns a region for every panel the band crosses seen from above, each with the at that puts it in the same place on the car and the constraint { stripe: <name> } that holds the pieces together. Use them as they are, adding a color. A piece you write yourself must carry the same constraint, so it is measured: check_fitment reports stripe-across for a piece that runs across the car, stripe-offset, in millimetres, where two pieces do not line up, and stripe-gap for a stretch of bodywork the stripe leaves bare, naming the panels that could carry it.
${aero ? AERO : ''}${wheels ? WHEELS : ''}- Some cars' bodywork is more than one texture, and each has its own panel names: a region on surfaces.body is drawn on every texture of it, so "left_mid" would land on each texture's left_mid. find_space picks the texture itself, lays a stripe or kit over all of them, and puts "role" on each region it returns when the surface has more than one: keep it, and the region is drawn on that texture alone. A region you place from its spots takes the same "role". Put the base color and everything else on surfaces.body; never paint a texture of it again through paint.<role>, which check_fitment refuses.
- A region with neither panel nor tags covers the whole texture. A "fill" like that, first in the list, is the base color.
- Use the colors the brief asks for, or the ones the style it names uses, and no others. A team name is not a color scheme: a Gulf livery is powder blue and orange, and a pink accent added because the team is called Neon Doll makes it not Gulf.
- Colors are palette names: set-palette first, one color per name, as { "op": "set-palette", "name": "gulf-blue", "value": "#7BB3D9" }; then use the name as "color". Identity values (set-identity: number, team, driver) are used in text as "{number}", "{team}".
...
- Names and numbers must read from trackside, which is further than it looks in a render. Give a team or driver name a box at least 120 mm tall on the car, a race number far more, and declare minMm on it so the measurement holds you to that. check_fitment measures the letters themselves and reports too-small when a race number's capitals are under 140 mm tall on the car or a name's under 45 mm. Text is shrunk to fit its box's width, so a long name in a narrow box comes out small however tall the box is: widen the box, or split the name over two lines, rather than making it taller. Color it to contrast hard with what is under it: dark on a light base, white on a dark one. check_fitment holds a team or driver name to 6:1 against what is under it and other lettering to 4.5:1: on Gulf blue that is navy close to black, not a mid navy. Never put a patch or band of color behind a name or a number to make it read: it looks amateur, and a person rejected run 23's for it. The one band lettering may sit on is a stripe that runs the car's whole length, laid out and declared as a stripe below. Keep it clear of shut lines and fittings.
- Unless the brief says otherwise, the number and the team name go on each side of the car as one group, on its main side panel (the door, or a formula car's sidepod), laid out the same on both sides: a roundel with the number inside it and the team name directly below. Do not size or place this group yourself. Call find_space with { layout: { number, name }, side: "left", marginMm: 30 } and the same with side: "right", both in one turn: it picks the side's main panel and says which, tries the group's proportions and the name on one line or two, sizes the letters by the arithmetic check_fitment measures them with, and returns the regions (roundel, number, and one or two name lines, each with panel, at, and scale where it matters) and the capital heights they measure. Use those regions as they are, adding only an id (roundel-left, number-left, team-left, and team-left-2 for a second line), a color, and constraints: minOnCar 1, minVisible 1 and minMargin set to the layout's marginMm on the roundel, and { groupWith: <the number's id on the same side> } on each name line, so check_fitment reports ungrouped if they part. Resizing the group by hand is how earlier runs spent most of a round and still ended with a number too small to read. Draw no second ring around the roundel: the layout gave that room to the letters.
- A stripe or color field is interrupted wherever the car has glass, vents, louvres or grilles, because paint does not go on a hole. That gap is not a fault: never shorten or delete a stripe to close one. A name on a rear quarter, a roof or a bumper is not where a spectator looks for it, and the critic does not count it as present.
...
- palette_ok: the colors are the ones the brief asks for or the style it names uses, or suit it if it names none, with enough contrast between artwork and base. An accent color that neither the brief nor its style has fails it: a Gulf livery is blue and orange, and a pink stripe added for the team's name is not Gulf.
- requirements: one entry for each separate thing the brief explicitly asks for: a color scheme, a number, each name or sponsor, a placement. When the brief names a style (a famous livery, a team's colors, an era), also list each of that style's signature elements as its own entry, such as the stripe it is known for and where each of its colors goes, because the colors alone are not the style. Requirements come from the brief and the style it names, never from the designer's account of the design: what the designer chose to add is not something the brief asked for. "present" is true only if you can see it on the car in these renders, and for a team or driver name only if it is where a spectator looks for it, beside the race number on the doors (or a formula car's sidepods); a name tucked on a rear quarter, a roof or a bumper is not present. "where" says in which view and on which part of the car, or "not visible".
...
export function verdictOf(text) {
  ...
  // bound to honor response_format, and "present: true" with no "where" is
