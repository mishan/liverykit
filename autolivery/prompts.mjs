...
- Unless the brief says otherwise, the number and the team name go on the doors as one group, laid out the same on both sides: a roundel with the number inside it, and the team name directly below the roundel, centred under it, its letters clear of the disc. Declare the group so it is measured: give each name constraints { groupWith: <the number's id on the same side> }, e.g. team-left with number-left, and check_fitment reports ungrouped when they are not on the same panel. Make the group as big as the door allows, not a guess at a size: call find_space with largest: true, aspect 0.85 and marginMm 30, which sweeps sizes and returns the largest group that fits whole, and where. Inside that rectangle the roundel is 55% of the group's width, at the top and centred, and the name spans the full width in the bottom 25% of its height, which leaves a clear gap between the disc and the letters. The number's box is a square centred on the roundel, 70% of its diameter; never give it the roundel's own box, whose corners are outside the disc, so the number's letters would cross the rim. Use the size the sweep returns; a group smaller than the door allows reads as timid from trackside.
