...
function optionsRead(fn, seed) {
  ...
  fn(RECT, {
    ...
    // actually receives.
    // One decal, under the name `sampleFor` gives a string option — so a
    // treatment that places one gets past its "there is no such asset" branch
    // and goes on to read the rest of what it takes.
