...
export function wholeModelGeometry(model, files, { livery = {}, profile = {} } = {}) {
  ...
  //
  // So both are kept and TAGGED, and each renderer draws exactly one of them.
  //
  // Both renderers pick the HIGH-DETAIL one, from every angle — which is NOT
  // what the game does. The game swaps to LR for an external camera, and this
  // was briefly faithful about that; faithful was worse. LR is a different mesh
  // with its own materials, and on this car those state ksAmbient and ksDiffuse
  // of 0.1: sensible in AC, which lights an interior with an ambient term
  // neither renderer here models, and near-black in both of them. It carries
  // none of the bakes either, so the lettering printed on the seat goes with
  // it. The views exist to show someone what they painted.
  //
  // Tagging rather than dropping LR outright — which this did first — because
  // dropping it loses a fact the rest of the editor needs: LR is where this
  // car's `interior` role lives, `Cockpit_LR_Colour.dds` is on no other mesh,
  // and a design painting the interior was left with nothing to paint it on.
  // Kept and tagged, the fitment report can still say so. (It does not yet —
  // see docs/backlog.md.)
  //
