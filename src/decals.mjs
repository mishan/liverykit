...
/**
 * And it is checked TWICE, against two different numbers.
 *
 ...
 * and so a test can reach this branch without generating fifteen megabytes of
 * noise to do it.
 */
 ...
 */
export async function loadDecals(dir, { log = () => {}, maxBytes = MAX_BYTES } = {}) {
  ...
  for (const entry of [...entries].sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (size > maxBytes) {
      log(`  ! ${entry.name} is ${kb(size)}, over the ${kb(maxBytes)} a decal may be, ` +
          'and was skipped');
      continue;
    }
    ...
    // THE SECOND CHECK, on what will actually be inlined — see MAX_BYTES. For a
    // PNG or a JPEG this is the file again and says nothing new; for an SVG it
    // is the only one of the two that has met the pixels.
    //
    // Skipped rather than re-rendered smaller: a decal that quietly arrived at
    // half the resolution it asked for would be this module choosing what
...
/**
 * Sizes a person can compare at a glance, which "0 KB" is not — and which
 * "8192 KB" is not either, now that this also has to print the cap.
 */
const kb = (n) => {
  if (n < 1024) return `${n} B`;
  if (n < 1024 * 100) return `${(n / 1024).toFixed(1)} KB`;
