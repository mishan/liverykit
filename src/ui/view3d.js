...
export function capped(w, h, max = 4096) {
  let a = w, b = h;
  while ((a > max || b > max) && a > 1 && b > 1) { a = Math.round(a / 2); b = Math.round(b / 2); }
  return [a, b];
}
...
export function textureSizes(surfaces, { budget = 192 * 1024 * 1024, max = 4096 } = {}) {
  ...
  let shrink = 1;
  const cost = () => want.reduce((n, [w, h]) =>
    n + Math.max(64, w / shrink) * Math.max(64, h / shrink) * 4, 0);
  while (cost() > budget && shrink < 16) shrink *= 2;
  return want.map(([w, h]) => ({
    w: Math.max(64, Math.round(w / shrink)),
    h: Math.max(64, Math.round(h / shrink)),
  }));
}
...
/**
   ...
   *
   * `MAX_TEXTURE_SIZE` is 4096 on plenty of hardware and a car can ship a 4096
   * sheet, so asking for the texture's real size is not by itself safe. Halved
   * rather than clamped to the limit exactly, to keep powers of two intact.
   */
